...
export function ChatBotButton({ onClick, hasUnread = false }: ChatBotButtonProps) {
  ...
  return (
    <div className="fixed bottom-6 right-4 z-50 flex flex-col items-end gap-2">
      {showTooltip && (
        <div className="bg-white text-gray-800 text-xs font-medium px-3 py-2 rounded-lg shadow-lg animate-fade-up whitespace-nowrap">
          Precisa de ajuda?
      ...
      <button
        type="button"
        onClick={onClick}
        className="group relative w-14 h-14 rounded-full bg-[#F5B800] text-black shadow-lg hover:bg-[#FFD040] hover:shadow-xl transition-all duration-300 flex items-center justify-center"
